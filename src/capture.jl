export get_info,get_image,capture_setup,capture_stop,capture_get_fileno,capture_dequeue,capture_enqueue,is_corrupt,capture_set_callback
...
function capture_setup(camera::Camera,num_dma_buffers::Int=1,flags::dc1394capture_flags_t=CAPTURE_FLAGS_DEFAULT)
    ccall((:dc1394_capture_setup,libdc1394),dc1394error_t,(Ptr{dc1394camera_info_t},UInt32,dc1394capture_flags_t),camera.handle,num_dma_buffers,flags)
end

function capture_stop(camera::Camera)
    ccall((:dc1394_capture_stop,libdc1394),dc1394error_t,(Ptr{dc1394camera_info_t},),camera.handle)
end

function capture_get_fileno(camera::Camera)
    ccall((:dc1394_capture_get_fileno,libdc1394),Cint,(Ptr{dc1394camera_info_t},),camera.handle)
end
...
function capture_dequeue(camera::Camera,policy::dc1394capture_policy_t=CAPTURE_POLICY_WAIT)
    ...
    ccall((:dc1394_capture_dequeue,libdc1394),
          dc1394error_t,(Ptr{dc1394camera_info_t},dc1394capture_policy_t,Ptr{Ptr{dc1394video_frame_t}}),
...
function capture_enqueue(frame::VideoFrame)
    if frame.handle!=C_NULL
        ccall((:dc1394_capture_enqueue,libdc1394),dc1394error_t,(Ptr{dc1394camera_info_t},Ptr{dc1394video_frame_t}),get_info(frame).camera,frame.handle)
    end
end

function is_corrupt(camera::Camera,frame::VideoFrame)
    val=ccall((:dc1394_capture_is_frame_corrupt,libdc1394),dc1394bool_t,(Ptr{dc1394camera_info_t},Ptr{dc1394video_frame_t}),camera.handle,frame.handle)
    val==TRUE
end

function capture_set_callback(camera::Camera,callback::Ptr{Void},data::Ptr{Void})
     ccall((:dc1394_capture_set_callback,libdc1394),Void,(Ptr{dc1394camera_info_t},Ptr{Void},Ptr{Void}),camera.handle,callback,data)
end
function capture_set_callback(callback::Function,camera::Camera)
    ...
    ccallback=cfunction(mycallback,Void,(Ptr{dc1394camera_info_t},Ptr{Void}))
     ccall((:dc1394_capture_set_callback,libdc1394),Void,(Ptr{dc1394camera_info_t},Ptr{Void},Ptr{Void}),camera.handle,ccallback,C_NULL)
end
